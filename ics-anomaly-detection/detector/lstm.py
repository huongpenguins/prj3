    ...
    def train(self, Xtrain, Ytrain, use_callbacks=False, **train_params):
        ...
        if True:
            ...
            df = pd.DataFrame({'epoch': epochs, 
                               ...
                               'val_loss': val_loss }) 
            save_dir = f'history/lstm-train-history-{self.params["layers"]}l-{self.params["units"]}u-{history}h' 
            os.makedirs(save_dir, exist_ok=True) 
            
            csv_path = os.path.join(save_dir, f'lstm-train-history-{self.params["layers"]}l-{self.params["units"]}u-{history}h.csv') 
            ...
            plt.grid(True) 
            plt.tight_layout() 
            plot_path = os.path.join(save_dir, f'lstm-train-history-{self.params["layers"]}l-{self.params["units"]}u-{history}h.png') 
